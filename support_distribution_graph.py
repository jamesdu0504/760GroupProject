...
def support_graph_distribution(frequent_itemsets, min_support, name):
    ...
    plt.savefig("supports/"+name+".svg", format="svg")
    plt.close()
...
def dual_support_graph_distribution(frequent_itemsets_A, frequent_itemsets_B, min_support, name):
    ...
    #Save the data for reconstruction later
    frequent_itemsets_B.to_csv("frequent_itemsets/FI_"+name+".csv")

    plt.legend()

    plt.ylabel("Number of frequent itemsets")
    plt.xlabel("Support threshold")
    plt.savefig("supports/"+name+"_dual.svg", format="svg")
    plt.close()
